    ...
    if (feeds.length > 0) {
      ...
      for (i = 0; i < feeds.length; i++) {
        ...
        html +=
          '<a class="link" href="' +
          feeds[i].url +
          '" title="Open feed URL" data-tabtitle="' +
          tab.title +
          '" target="_blank">' +
          feeds[i].title +
          "</a>";
        html +=
          '<span class="feed-url">' + truncate(feeds[i].url, 50) + "</span>";
        html += "</td>";
        html += '<td class="feed-copy">';
        html +=
          '<a class="copyButton copyLink" title="Copy feed URL" href="#">Copy URL</a>';
        html += "</td>";
        html += "</tr>";
      }
      html += "</table>";
      ...
      html +=
        '<a id="copyAllLinks" class="" title="Copy all feeds URLs" href="#">Copy all URLs</a>';
      html += "</div>";
      ...
      for (let i = 0; i < copyButtons.length; i++) {
        copyButtons[i].addEventListener("click", function () {
          ...
          copyToClipboard(url, {
            type: "success",
            title: tabTitle,
            message: "Feed URL copied in clipboard!",
          });
        });
      }
      ...
      copyButtonAll.addEventListener("click", function () {
        var feeds_list = document
          .getElementById("feeds-list")
          .querySelectorAll(".feed-title a.link");
        ...
        copyToClipboard(textToCopy, {
          type: "success",
          title: "",
          message: "Feeds URLs copied in clipboard!",
        });
      });
    } else {
